...
chat_model = ChatDatabricks(endpoint=foundation_model_name, temperature=0.5, max_tokens=2000)
...
with mlflow.start_run():
    ...
    search_spec={"k":3, "score_threshold": 0.5} # This is to instantiate the VS and can later be overwritten in filter_custom
    with warnings.catch_warnings():
        warnings.filterwarnings("ignore", category=UserWarning, module="langchain_core.callbacks.manager")
        ...
        filter_custom = {"k":20, "score_threshold": 0.5, "filters":{"filter1":["condition1", "condition2"], "filter2": "condition"}}
        ...
        for ii in range(len(questions)):
            ...
            query=questions[ii]
            question_filtered = {"query": query, "search_kwargs": filter_custom}
        ...
        source_documents = [answers[i]["source_documents"] for i in range(len(answers))]
        ...
        result_df.columns = ["results", "times", "source_documents", "prompt", "model", "embedding_model", "questions", "search_config"]
...
with mlflow.start_run(run_name="all_studies_combined") as run:
    ...
    model_info = mlflow.langchain.log_model(
        qa,
        loader_fn=load_filter_retriever,  # Load the retriever with DATABRICKS_TOKEN env as secret (for authentication).
        artifact_path="chain",
